...
install_nvm() {
    ...
    # We use bash -i to have $NVM_DIR loaded
    bash -i -c 'echo -e "yarn\npnpm" >> $NVM_DIR/default-packages'
    cd ~-
}
...
# Install micromamba in ~/.local/bin
# and enable shell to use it right after installation
install_micromamba() {
...
# Setup a micromamba environment and optionally activate it
# Expected args:
# 1. the environment name to create (will override environment name in environment file if any)
# 2. the environment file to use (default: environment.yml in current directory)
# 3. whether to automatically activate the environment (default: false)
...
# NOTE: this function should be run in the root directory of the python project to test
run_pytest() {
...
# Run tests for a python library module
# Expected arguments
# 1. Root directory
# 2. true to publish code coverage to sonar
